...
"""
    ...
    print("Услуги добавлены, ну хоть что-то")
...
def add_discoveries():
    ...
    for _ in range(30):
        ...
        discovery.description = random_text()
        discovery.year = random.randint(1000, 2000)
        discovery.status = random.randint(2, 5)
        discovery.owner = random.choice(users)

        if discovery.status in [2, 3, 4]:
            discovery.verify = random.randint(0, 1)

        if discovery.status in [3, 4]:
            discovery.date_complete = random_date()
            discovery.date_formation = discovery.date_complete - random_timedelta()
            discovery.date_created = discovery.date_formation - random_timedelta()
            discovery.moderator = random.choice(moderators)
        else:
            discovery.date_formation = random_date()
            discovery.date_created = discovery.date_formation - random_timedelta()
...
def add_orders():
    ...
    print("Заявки добавлены и СЛАВА БОГУ")
